...
namespace BH_STG.BarrageEngine.ParticleSystem
{
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        protected ParticleSystem(Main game, int howManyEffects)
            : base(game)
        {
            this.game = game;
        ...
        public void AddParticles(Vector2 where, Random rand)
        {
            ...
            for (int i = 0; i < numParticles && freeParticles.Count > 0; i++)
            {
                ...
                InitializeParticle(p, where, rand);
            }
        }
        ...
        /// </param>
        protected virtual void InitializeParticle(Particle p, Vector2 where, Random rand)
        {
            // first, call PickRandomDirection to figure out which way the particle
            // will be moving. velocity and acceleration's values will come from this.
            Vector2 direction = PickRandomDirection(rand);

            // pick some random values for our particle
            float velocity = minInitialSpeed + (float)rand.NextDouble() * (maxInitialSpeed - minInitialSpeed),
                  acceleration = minAcceleration + (float)rand.NextDouble() * (maxAcceleration - minAcceleration),
                  lifetime = minLifetime + (float)rand.NextDouble() * (maxLifetime - minLifetime),
                  scale = minScale + (float)rand.NextDouble() * (maxScale - minScale),
                  rotationSpeed = minRotationSpeed + (float)rand.NextDouble() * (maxRotationSpeed - minRotationSpeed);

            // then initialize it with those random values. initialize will save those,
            // and make sure it is marked as active.
            p.Initialize(where, velocity * direction, acceleration * direction,
                         lifetime, scale, rotationSpeed, rand, game);
        }
        ...
        /// circular pattern.
        /// </summary>
        protected virtual Vector2 PickRandomDirection(Random rand)
        {
        ...
        {
            if (game.state == Main.GameStates.game)
            {
        ...
        public override void Draw(GameTime gameTime)
        {
            if (game.state == Main.GameStates.game)
            {
                ...
                foreach (Particle p in particles)
                {
                    ...
                    Vector2 coords = p.Position;
                    if (IsFlipped)
                        coords.Y = -coords.Y + 780;

                    game.spriteBatch.Draw(texture, coords, null, color,
